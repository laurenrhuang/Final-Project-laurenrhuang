...
A quick definition here, AUC is the "area under the curve." ROC and AUC represent the model's capability of distinguishing between classes. In our scenario, AUC is measuring how capable each model is at distinguishing students who "Fail" from students who "Pass."
...
```{r, message = F, warning = F}
# ROC curve Method 1. merged dataset
augment(final_fit, new_data = grades_test_fctr) %>%
...
# ROC curve Method 2. Portuguese for training, Maths for testing
augment(final_fit2, new_data = grades_test2_fctr) %>%
...
The highest accuracy we get is 0.8972186, but we get an ROC AUC of 1. Why do you think this is? This may be because the ROC is biased and performs well on one class, but has some false negatives as well.
