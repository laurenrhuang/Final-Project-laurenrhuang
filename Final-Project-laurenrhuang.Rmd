...
Now let's do the same for Method 2.
...
A quick definition here, AUC is the "area under the curve." We'll see what this "curve" is referring to the code blocks below. ROC and AUC represent the model's capability of distinguishing between classes. In our scenario, AUC is measuring how capable each model is at distinguishing students who "Fail" from students who "Pass."
...
By comparing the testing accuracy, the model that used Method 1 actually gave higher accuracy than Method 2. (The accuracy from Method 1 is 0.9235669, the accuracy from Method 2 is 0.8936709) The difference between the two accuracies is only about 0.03. This is opposite of what we saw when we used the training data. Let's plot their respective ROC curves to see if the visual results are consistent.
...
This is an example of an ROC curve. For reference, this website's definition says "A ROC curve is constructed by plotting the true positive rate (TPR) against the false positive rate (FPR)." https://www.displayr.com/what-is-a-roc-curve-how-to-interpret-it/
...
We can look at all the different combinations of mixture and penalty parameters, and select the one that gives us the best accuracy from the training data. Then, we fit it to the testing data.
...
We follow the same steps for Method 2.
...
From the results above, it seems like Method 1 gives us better accuracy when fitted to the elastic net model. Let's investigate the details of this model by calculating AUC, roc_auc, and plotting ROC curves. We can also make a heatmap of the confusion matrix.
...
Our results match up, hooray! Our ROC AUC is 0.9799, which follows the ROC curve we plotted. This can also be verified with the results from the confusion matrix. Lastly, our metrics tell us that the elastic net model accuracy on the testing dataset is 0.9299363. Are there any other models that can give us even better results?
...
Again, this model can take a long time to run, so we save it to a file.
...
Next, we plot a comparison of the cost-complexity parameter to accuracy. What do you notice? It looks like accuracy peaks and flattens at about 0.92 once we reach a cost complexity of about 0.02.
...
In this case, the best accuracy is 0.9014038. Using the corresponding best cost complexity parameter gives us an ROC AUC of 0.9048427.
...
Here, the best accuracy is 0.9029647. Using the corresponding best cost complexity parameter gives us an ROC AUC of 0.9338024.
...
Again, we have a lower accuracy of 0.8981952 compared to the decision tree models, and the ROC AUC is 1 again. Let us fit the boosted tree model to our testing data and see how it performs.
...
Looking at specifically classification models and overall AUC and accuracy, the Elastic Net model using Method 1. merged dataset performed the best, with a ROC AUC of 0.9799213 and accuracy of 0.9299363 on the testing data.
