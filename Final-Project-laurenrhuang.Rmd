...
From the three kinds of models, it seems like logistic regression performed the best, giving us the highest accuracy. For logistic regression and QDA, Method 2 (Portuguese=training, Maths=testing) of data splitting gave higher accuracy. But for QDA, Method 1 (merged dataset) gave a higher accuracy.


### k-fold cross validation

We can try k-fold cross validation on the model that performed best out of classification models (logistic regression)
...
log_acc_kfold <- augment(final_fit, new_data = grades_test_fctr) %>%
  accuracy(truth = grades_test_fctr$G3, estimate = .pred_class)
log_acc_kfold
```
